...
*/
const char op1[][10]={"    ","sum ","sqrt","!   ", ".   ",".~  "};     // common operators.
...
int generate(char *a);                 // generate as much as possible pre-fix expression from string a
...
void search()
{
  ...
  for(i = 0;i < OP_MAX;i++){
	...
	if(generate(a)) goto print_result;									// generate as much as possible prefix expression from the above expression
	sprintf(a,"44%c4%c4%c\0",op[i],op[j],op[k]); 	// above process is repeated 4 more times.
	if(generate(a)) goto print_result;
