...
#define  RANGE 200            	// Range of results

/* KIND 2: these operators required two arguments
   +: addition, -: subtraction, *: multiply, \: division, %: mod, ^: power*/
const char op[]="+-*/%^\0";                         
#define OP_MAX 6                // the number of KIND 2's operators
                

/* KIND 1: these operators required only one arguments
   sqrt: square of root , sum: eg: sum(4)=1+2+3+4*, !: factorial
   ATTENTION: this include "space" operator, which also means we do nothing
...
*/
const char op1[][10]={"    ","sqrt","!   ","sum "};
#define OP_MAX1 4               // the number of KIND 2's operators
